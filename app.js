(() => {
  ...
  // --- Switches & Regulation (optional allow-list) ---
  function loadSwitchesFromStorage(){
    try{
      const nr = localStorage.getItem(NORMAL_RULES_KEY);
      if (nr !== null) state.ui.normalRules = (nr === "true");
      const nl = localStorage.getItem(NO_LEGENDS_KEY);
      if (nl !== null) state.ui.noLegends = (nl === "true");
      const re = localStorage.getItem(REG_ENABLED_KEY);
      if (re !== null) state.ui.regEnabled = (re === "true");
      const raw = localStorage.getItem(REG_CSV_KEY);
      if (raw) state.reg.rawCsv = raw;
    }catch{}
  }

  function persistSwitches(){
    try{
      localStorage.setItem(NORMAL_RULES_KEY, String(!!state.ui.normalRules));
      localStorage.setItem(NO_LEGENDS_KEY, String(!!state.ui.noLegends));
      localStorage.setItem(REG_ENABLED_KEY, String(!!state.ui.regEnabled));
    }catch{}
  }
  ...
  function splitCsvLine(line){
    // Simple CSV split (quotes supported). Enough for our small allow-list CSV.
    const out = [];
    let cur = "";
    let q = false;
  ...
  function rebuildNumIndex(){
    // National Dex No. -> list of showdown IDs
    state.numToIds = new Map();
    if (!state.pokedex) return;
    for (const [rawId, p] of Object.entries(state.pokedex)){
      const sid = normalizeId(rawId);
      const num = p?.num;
      if (!num) continue;
      if (!state.numToIds.has(num)) state.numToIds.set(num, []);
      state.numToIds.get(num).push(sid);
    }
  }

  function pickBaseIdFromNum(num, wantName=""){
    const ids = state.numToIds?.get(num) || [];
    if (!ids.length) return "";
    const wn = normalize(wantName);
    if (wn){
      // match Japanese label or English name
      for (const id of ids){
        const ja = fmtSpecies(id);
        if (normalize(ja) === wn) return id;
        const en = state.pokedex?.[id]?.name || "";
        if (normalize(en) === wn) return id;
      }
    }
    // base form (no baseSpecies, no forme)
    for (const id of ids){
      const p = state.pokedex?.[id];
      if (p && !p.baseSpecies && !p.forme) return id;
    }
    return ids[0];
  }
  ...
  function updateRegInfo(){
    ...
    if (!e) return;
    if (!state.reg.rawCsv){
      e.textContent = "レギュ: 未設定";
      return;
    }
    if (!state.dexLoaded){
      e.textContent = "レギュ: 読込待ち（図鑑読み込み後に反映）";
      return;
    }
    if (state.reg.loaded){
      e.textContent = `レギュ: 読込済み（${state.reg.allowed.size}匹）`;
    } else {
      e.textContent = "レギュ: 読込失敗（CSVを確認）";
    }
  }

  function parseRegulationCsv(text){
    state.reg.rawCsv = String(text || "");
    try{ localStorage.setItem(REG_CSV_KEY, state.reg.rawCsv); }catch{}

    // If dex isn't loaded yet, just keep raw and parse later.
    if (!state.dexLoaded){
      state.reg.loaded = false;
      state.reg.allowed = new Set();
      state.reg.unknown = [];
      updateRegInfo();
      return;
    }
    ...
    const idx = (name) => headerNorm.indexOf(normalize(name));

    const idxId = idx("id");
    const idxNo = idx("no");
    const idxName = (idx("名前") >= 0) ? idx("名前") : idx("name");

    let idxAllow = idx("allow");
    if (idxAllow < 0) idxAllow = idx("採用");
    if (idxAllow < 0) idxAllow = idx("レギュ");
    if (idxAllow < 0) idxAllow = header.length - 1; // fallback

    for (let li=1; li<lines.length; li++){
      const cols = splitCsvLine(lines[li]);
      const allow = parseBool(cols[idxAllow]);
      if (!allow) continue;

      let sid = "";
      if (idxId >= 0 && cols[idxId]){
        sid = normalizeId(cols[idxId]);
      } else if (idxNo >= 0 && cols[idxNo]){
        try{
          const num = parseInt(cols[idxNo], 10);
          const nm = (idxName >= 0 ? (cols[idxName] || "") : "");
          sid = pickBaseIdFromNum(num, nm);
        }catch{
          sid = "";
        }
      } else if (idxName >= 0 && cols[idxName]){
        const wn = normalize(cols[idxName]);
        for (const opt of (state.speciesOptions || [])){
          if (normalize(opt.ja) == wn || normalize(opt.en) == wn){
            sid = opt.id; break;
          }
        }
      }
      ...
      if (sid){
        state.reg.allowed.add(sid);
      } else {
        state.reg.unknown.push(lines[li].slice(0, 200));
      }
    }

    state.reg.loaded = state.reg.allowed.size > 0;
    updateRegInfo();
  }

  async function loadRegulationFromUrl(url){
    const res = await fetch(url, {cache:"no-cache"});
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const text = await res.text();
    parseRegulationCsv(text);
    return true;
  }
...
// --- Dex loading ---
  ...
  async function loadDex() {
    ...
    const ids = ["#btnClearPicks", "#btnAutoLeft", "#btnAutoRight", "#btnAutoBoth", "#toggleHideRight", "#btnAutoTeamLeft", "#btnClearTeamLeft", "#btnAutoTeamRight", "#btnClearTeamRight", "#toggleLearnset", "#toggleNormalRules", "#toggleNoLegends", "#toggleReg", "#fileRegCsv", "#btnLoadRegDefault"];
  ...
  // --- Filters (normal rules / regulation allow-list) ---
  function isLegendOrMyth(id){
    const sid = normalizeId(id);
    const p = state.pokedex?.[sid] || state.pokedex?.[id] || null;
    if (!p) return false;
    let tags = Array.isArray(p.tags) ? p.tags : [];
    // Some forms miss tags; fall back to base species tags.
    if ((!tags || !tags.length) && p.baseSpecies){
      const baseId = normalizeId(p.baseSpecies);
  ...
  function isAllowedByNormalRules(id){
    ...
    if (state.ui.noLegends) return !isLegendOrMyth(id);
    return true;
  }
  ...
  function isAllowedByRegulation(id){
    ...
    return state.reg.allowed.has(normalizeId(id));
  }

  function isAllowedSpecies(id){
    const sid = normalizeId(id);
    return isAllowedByNormalRules(sid) && isAllowedByRegulation(sid);
  }

  function getAllowedSpeciesOptions(){
    if (!state.dexLoaded) return [];
    if (!state.ui.normalRules && !state.ui.regEnabled) return state.speciesOptions || [];
    return (state.speciesOptions || []).filter(o => isAllowedSpecies(o.id));
  }

  function getAllowedSpeciesIds(){
    return getAllowedSpeciesOptions().map(o => o.id);
  }
  ...
  function renderMonCard(side, idx, mon) {
    ...
    const speciesBox = createSearchBox({
      ...
      getOptions: () => getAllowedSpeciesOptions(),
      onPick: (o) => {
        mon.speciesId = o.id;
        // default ability
  ...
  async function importJson(file) {
    ...
    if (obj.ui && typeof obj.ui === "object") {
      state.ui.hideRightPicks = !!obj.ui.hideRightPicks;
      state.ui.normalRules = (obj.ui.normalRules !== undefined) ? !!obj.ui.normalRules : state.ui.normalRules;
      state.ui.noLegends = (obj.ui.noLegends !== undefined) ? !!obj.ui.noLegends : state.ui.noLegends;
      state.ui.regEnabled = (obj.ui.regEnabled !== undefined) ? !!obj.ui.regEnabled : state.ui.regEnabled;
    }
    state.teams = obj.teams;

    // regulation CSV (optional)
    if (typeof obj.regCsv === "string" && obj.regCsv.trim()) {
      parseRegulationCsv(obj.regCsv);
    }
    ...
    // sync UI
    try{
      const tL = $("#toggleLearnset"); if (tL) tL.checked = !!state.filterLearnset;
      const tNR = $("#toggleNormalRules"); if (tNR) tNR.checked = !!state.ui.normalRules;
      const tNL = $("#toggleNoLegends"); if (tNL) { tNL.checked = !!state.ui.noLegends; tNL.disabled = !state.ui.normalRules; }
      const tR = $("#toggleReg"); if (tR) tR.checked = !!state.ui.regEnabled;
      updateRegInfo();
    }catch{}
  ...
  function getAutoSpeciesPool(){
    ...
    for (const id of ids) {
      ...
      if (!isAllowedSpecies(id)) continue;
      out.push(id);
    }
  ...
  try{
    ...
    const rg0 = $("#toggleReg"); if (rg0) rg0.checked = !!state.ui.regEnabled;
    updateRegInfo();
    if (nl0) nl0.disabled = !state.ui.normalRules;
  }catch{}
  ...
  if (normalRules) {
    normalRules.addEventListener("change", (e) => {
      ...
      // 通常ルールOFFなら「伝説除外」は無効化（値は保持）
      const nl = $("#toggleNoLegends");
      if (nl) nl.disabled = !state.ui.normalRules;
      persistSwitches();
      renderAll();
    });
  }
  ...
  if (regToggle) {
    regToggle.addEventListener("change", async (e) => {
      ...
      // ONにする時: レギュが未読込なら同梱CSVを試す
      try {
        if (!state.reg.loaded) {
          if (state.reg.rawCsv) {
            parseRegulationCsv(state.reg.rawCsv);
          } else {
            await loadRegulationFromUrl(DEFAULT_REG_URL);
          }
        }
        if (!state.reg.loaded) throw new Error("レギュCSVの読込に失敗しました");
        state.ui.regEnabled = true;
        persistSwitches();
        renderAll();
      } catch (err) {
        console.error(err);
        setStatus(`レギュ適用できません: ${err.message}`, "err");
        state.ui.regEnabled = false;
        e.target.checked = false;
        persistSwitches();
        renderAll();
  ...
  if (regFile) {
    regFile.addEventListener("change", async (e) => {
      ...
      try {
        ...
        parseRegulationCsv(text);
        if (!state.reg.loaded) throw new Error("内容を確認してください");
        ...
        console.error(err);
        setStatus(`レギュCSV読込失敗: ${err.message}`, "err");
      } finally {
        e.target.value = "";
      }
    });
  ...
  if (btnRegDefault) {
    btnRegDefault.addEventListener("click", async () => {
      try {
        await loadRegulationFromUrl(DEFAULT_REG_URL);
        if (!state.reg.loaded) throw new Error("内容を確認してください");
        setStatus(`同梱レギュを読み込みました（${state.reg.allowed.size}匹）`, "ok");
        updateRegInfo();
        renderAll();
      } catch (err) {
        console.error(err);
        setStatus(`同梱レギュ読込失敗: ${err.message}`, "err");
      }
    });
  }
